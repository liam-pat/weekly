...
Completed records through 2026-09-09, moved from `process.md` on 2026-09-13.
Read only when a task needs the history of a related feature.
See [process.md](process.md) for current work and [../AGENTS.md](../AGENTS.md)
for execution rules.

These records describe behavior and validation at the time of completion.
Later changes may supersede them; historical page counts are not current test
expectations. In particular, the current configured hostname is
`weekly.apartment.internal`; older entries retain the earlier spelling.
